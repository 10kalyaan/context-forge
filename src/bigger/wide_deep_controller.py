...
def safe_session_order_from_chunk(chunk):
    """
    Prefer explicit session_order if present.
    Fallback to parsing session_id only for formats like 'session_7'.
    """
    if chunk is None:
        return -1

    if "session_order" in chunk and chunk["session_order"] is not None:
        try:
            return int(chunk["session_order"])
        except Exception:
            pass
...
def extract_features(question, retrieved_summaries, retrieved_chunks, summary_answer, raw_answer):
    ...
    top_raw_session_order = safe_session_order_from_chunk(retrieved_chunks[0]) if len(retrieved_chunks) > 0 else -1

    num_summary_source_chunks = len(retrieved_summaries[0]["source_chunk_ids"]) if len(retrieved_summaries) > 0 else 0

    # LongMemEval summaries may not have source_dia_ids
    num_summary_source_dia_ids = len(retrieved_summaries[0].get("source_dia_ids", [])) if len(retrieved_summaries) > 0 else 0
    ...
    session_order_gap = (
        abs(top_summary_session_order - top_raw_session_order)
        if top_summary_session_order != -1 and top_raw_session_order != -1
        else -1.0
    )
