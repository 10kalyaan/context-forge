...
CHUNKS_PATH = Path("data/processed/lme_processed_chunks.json")
...
class EmbeddingMemoryRetriever:
    ...
    def _build_indices(self):
        print(f"Building LME embedding indices on device: {self.device}")

        for sample_id, chunks in self.chunks_by_sample.items():
            texts = [f"Dialogue chunk:\n{c['text']}" for c in chunks]
    ...
    def retrieve_chunks(self, sample_id: str, question: str, top_k: int = 3) -> List[Dict]:
        ...
        for idx in top_indices:
            ...
            results.append({
                ...
                "text": item["text"],
                "has_answer_flags": item.get("has_answer_flags", []),
            })
        return results
